...
export const meta = {
  ...
  lastEdited: '2019-06-12T09:56:11.000Z'
}
...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- |
| `req.query`        | an object containing the request [query string](https://en.wikipedia.org/wiki/Query_string)                                                                  | request  |
| `req.cookies`      | an object containing the parsed cookies sent with the request                                                                                                | request  |
| `req.body`         | an object containing the body sent with the request, or `null` if no body is sent                                                                            | request  |
| `res.status(code)` | a function to set the status code sent with the response. `code` must be a valid [HTTP status code](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes) | response |
| `res.send(body)`   | a function to set the content of the response. `body` can be a `string`, an `object` or a `Buffer`                                                           | response |
| `res.json(obj)`    | a function to send a JSON response                                                                                                                           | response |
...
- If the `Content-Type` header is set to `application/json`, the JSON sent with the request is parsed and `req.body` is an object containing it
- If the `Content-Type` header is set to `application/x-www-form-urlencoded`, the query string sent with the request is parsed and `req.body` is an object containing it

In the rest of the cases, `req.body` is a node buffer that contains the content sent with the request.

With the `req.body` helper, you can build applications without extra dependencies or having to [parse the content of the request manually](https://nodejs.org/ja/docs/guides/anatomy-of-an-http-transaction/#request-body). The following lambda example accepts a JSON body in the POST request, and returns a stringified version of it:

```js
module.exports = (req, res) => {
  if (req.method !== 'POST') {
    res.status(405)
    res.json({ error: 'only POST requests are accepted' })
    return
  }

  if (req.body) {
    res.status(400)
    res.json({ error: 'no JSON object in the request' })
    return
  }
...
<Caption>
  ...
  returns a stringified version of the JSON object sent in the request
</Caption>
