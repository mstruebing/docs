...
export const meta = {
  ...
  lastEdited: '2019-06-13T14:14:04.000Z'
}
...
With the `req.body` helper, you can build applications without extra dependencies or having to [parse the content of the request manually](https://nodejs.org/ja/docs/guides/anatomy-of-an-http-transaction/#request-body). The following example reads `req.body` when it is string, it expects `ping` and returns `pong`:
...
module.exports = (req, res) => {
  ...
  if (req.body !== 'ping') {
    return res.status(400).json('The value expected is `ping`')
  }
...
<Caption>
  ...
  returns <InlineCode>pong</InlineCode> when you send{' '}
