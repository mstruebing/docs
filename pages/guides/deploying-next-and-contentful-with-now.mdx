...
export const meta = {
  ...
  published: '2019-06-14T16:51:04.000Z',
  authors: ['msweeneydev'],
  ...
  lastEdited: '2019-06-14T16:51:02.000Z'
}

In this guide, you will discover how to create a [Next.js](https://nextjs.org/) app that displays links to [ZEIT blog posts](/blog) by utilizing the [Contentful](https://www.contentful.com/) client, before deploying **with just a single command** to [ZEIT Now](/docs/v2).
...
From your [Contentful Spaces dashboard](https://app.contentful.com/spaces/), create a new **Content Model** called **post** with the following fields, all of type **Short Text**:
...
Next, using the **Content** tab, using the **Add Post** button to create a post, providing the relevant details.
...
Firstly, it requires the `contentful` dependency and creates a client, this provides access to many useful helper methods.

Inside the `HomePage` function, two asynchronous functions are defined, these are then called in the `useEffect` hook to retrieve the posts.
...
With your project ready, the `now.json` file provides an opportunity to instruct [Now](/docs/v2) on how to build and deploy your project. Add a `now.json` file at the root of project directory with the following code:
...
The above [`now.json` file](/docs/v2/deployments/configuration/) achieves two things:
...
With your `now.json` file created, you should add a `next.config.js` file with the code below:
...
<Note>
  As recommended by Contentful, import your environment variables from a
  separate file, adding it to your <InlineCode>.gitignore</InlineCode> to
  prevent uploading by mistake.
</Note>
...
You will see a short build step in your terminal followed by the news that your project has been [deployed](https://zeit.co/docs/v2/deployments/basics/), it should look similar to this: <https://nextjs-contentful.now.sh/>
