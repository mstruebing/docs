...
By default the routing is handled like the filesystem of your deployment. If a user requests `/123.png`, if your deployment doesn't contain any routes with a valid `"src"` matching that path, it will fallback to the filesystem and serve `/123.png` if it exists.
